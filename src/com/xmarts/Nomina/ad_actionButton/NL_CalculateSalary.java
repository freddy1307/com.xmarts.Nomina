		...
		for(int x=0;x<ded.length;x++){
			...
			if(ded[x].tipopago.equals("S")){
			   	 ...
			   	 System.out.println("entro");
			   		...
			   		String isrRD= NLISR.isrrd(new DalConnectionProvider(),id_slab);
			   		
			   		System.out.println("ISR dominicano "+isrRD);
			        ...
			        if(isrs.charAt(0) == 'Y'){
			         ...
			         String id_subsidio = NLISR.valorsubsidio(new DalConnectionProvider(),id_slab);
			   		...
			   		SPMPayCompDeduct spca = OBDal.getInstance().get(SPMPayCompDeduct.class,ded[x].dedid);
			   		double res1 = 0;
			   		BigDecimal amountyear1 = new BigDecimal(0);
			   			if(isrRD.charAt(0) == 'Y'){
			   				for(int a=0;a< line.length;a++){
			   				double fromamt = new java.lang.Double(line[a].froma.toString());
			   			    double toamt = new java.lang.Double(line[a].toa.toString());
			   				...
			   				System.out.println("A CTC "+toamt);
			   					if(mntanual >= fromamt && mntanual <= toamt){
			   						...
			   						if(a==0){
			   							...
			   							if(a==1){
			   								dife = mntanual - fromamt;
			   								System.out.println("Diferencia "+dife);
			   								res1 = dife * (new java.lang.Double(line[a].porcen.toString()) / 100);
			   								System.out.println("Rango 2 "+res1);
			   							}else{
			   								if(a==2){
				   								dife = mntanual - fromamt;
				   								System.out.println("Diferencia "+dife);
				   								res1 = dife * (new java.lang.Double(line[a].porcen.toString()) / 100);
				   								res1 = res1 + 29994;
				   								System.out.println("Rango 3 "+res1);
			   								}else{
				   								dife = mntanual - fromamt;
				   								System.out.println("Diferencia "+dife);
				   								res1 = dife * (new java.lang.Double(line[a].porcen.toString()) / 100);
				   								res1 = res1 + 76652;
				   								System.out.println("Rango 4 "+res1);
			   								}
			   							}

			   						}//llave del else final
			   					}//llave del if que compara los montos con los rangos
			   					
			   				}//for
			   				amountyear1 = BigDecimal.valueOf(res1);
			   				spca.setAmountperYear(amountyear1);
			   			}else{
			        //System.out.println("Monto anual "+mntanual);
